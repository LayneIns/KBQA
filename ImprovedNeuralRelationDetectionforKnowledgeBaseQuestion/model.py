...
class HrbilstmModel:
	...
	def __init__(self, arg_config):
		...
		with tf.variable_scope("BiLSTM_embedding_layer_for_relation"):
			relation_seg_output_fw, relation_seg_output_bw = self.BiLstm_for_relation(self.relation_seg_embedding, self.sequence_length_seg, arg_config.relation_embedding_size)
			# [one_case_length, relation_embedding_size]
		with tf.variable_scope("BiLSTM_embedding_layer_for_relation", reuse=True):
			relation_seg_all_output_fw, relation_seg_all_output_bw = self.BiLstm_for_relation(self.relation_seg_all_embedding, self.sequence_length_seg_all, arg_config.relation_embedding_size)
			# [one_case_length, relation_embedding_size]

			self.relation_seg_repre = tf.concat([relation_seg_output_fw, relation_seg_output_bw], axis=2)
			# [one_case_length, relation_embedding_size*2]
			self.relation_seg_all_repre = tf.concat([relation_seg_all_output_fw, relation_seg_all_output_bw], axis=2)
			# [one_case_length, relation_embedding_size*2]
		...
		with tf.variable_scope("BiLstm_embedding_layer_for_question_1"):
			lstm_cell_fw = tf.contrib.rnn.GRUCell(arg_config.question_embedding_size)
			lstm_cell_bw = tf.contrib.rnn.GRUCell(arg_config.question_embedding_size)
			...
			self.question_repre_1 = tf.concat([output_fw_1, output_bw_1], axis=2)
		...
		with tf.variable_scope("BiLstm_embedding_layer_for_question_2"):
			...
			self.question_repre_2 = tf.concat([output_fw_2, output_bw_2], axis=2)
			# [one_case_length, question_embedding_size*2]
		...
		self.extended_question_repre = tf.matmul(self.question_matrix, self.question_repre_pooled_squeeze)
		# [one_case_length, question_embedding_size*2]

		self.normalize_question_repre = tf.nn.l2_normalize(self.extended_question_repre, 1)  
		# [one_case_length, question_embedding_size*2]      
		self.normalize_relation_repre = tf.nn.l2_normalize(self.relation_repre_pooled_squeeze, 1)
		# [one_case_length, question_embedding_size*2]
